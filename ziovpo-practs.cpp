...
namespace
{
    constexpr wchar_t WINDOW_CLASS_NAME[] = L"ZIoVPO_Pract1_Window";
    constexpr wchar_t WINDOW_TITLE[] = L"ZIoVPO — практика 1";
    constexpr wchar_t MUTEX_NAME[] = L"Local\\ZIoVPO_Pract1_SingleInstance";
    constexpr wchar_t TRAY_TIP[] = L"ZIoVPO: практика 1 (трей)";
...
bool AddTrayIcon(HWND hwnd, HICON icon)
{
    ...
    StringCchCopyW(g_nid.szTip, ARRAYSIZE(g_nid.szTip), TRAY_TIP);
...
bool CreateMainWindow(HINSTANCE hInstance, HICON& outIcon)
{
    WNDCLASSEXW wc{};
    wc.cbSize = sizeof(wc);
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR, int)
{
    ...
    if (!CreateMainWindow(hInstance, appIcon))
    {
        ReleaseSingleInstance();
        return 1;
    }

    if (!AddTrayIcon(g_hWnd, appIcon))
    {
        ReleaseSingleInstance();
